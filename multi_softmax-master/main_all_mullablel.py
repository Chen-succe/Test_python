...
from models import *
from util import *
from util.datasets import CustomDataset_multi
...
class FaceModel(torch.nn.Module):
    ...
    def __init__(self,num_classes,multi_head=False):
        ...
        self.head = torch.nn.Linear(1280, 5,bias=False)
        torch.nn.init.xavier_uniform_(self.head.weight)
        if self.multi_head:
            self.head_binary = torch.nn.Linear(1280, 1,bias=False)
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--train_list', default='/home/ailun.li/Multi_softmax/data_txts/198_质量模块/quality_all_train.txt', type=str,help='dataset path')
    ...
    parser.add_argument('--val_list', default='/home/ailun.li/Multi_softmax/data_txts/198_质量模块/quality_all_test.txt', type=str,help='dataset path')
    # /mnt/mfs2/ailun.li/data_txt/all/huanlian/newlabel/huanlian_val_newlabel.txt'
    # data_txts/all_test_PIL_resize224.txt
    parser.add_argument('--test_list', default='/home/ailun.li/Multi_softmax/data_txts/198_质量模块/quality_all_test.txt', type=str, help='dataset path')
    ...
    parser.add_argument('--bi_class_weight', default=[1,1], type=list, help="Binary classify weight")
    ...
    parser.add_argument('--opt-eps', default=1e-6, type=float, metavar='EPSILON',help='Optimizer Epsilon (default: 1e-8)')
    ...
    parser.add_argument('--clip-grad', type=float, default=10, metavar='NORM',help='Clip gradient norm (default: None, no clipping)')
...
def main(args):
    ...
    dataset_train = CustomDataset_multi(args.train_list, type='train', transforms=get_transform('train',resize=args.input_size))
    print(dataset_train)
    # add_imgs = []
    ...
    dataset_val = CustomDataset_multi(args.val_list, type='test', transforms=get_transform('test',resize=args.input_size))
    dataset_test = CustomDataset_multi(args.test_list, type='test', transforms=get_transform('test', resize=args.input_size))
    # dataset_test_other = CustomDataset(None, type='test', transforms=get_transform('test', resize=args.input_size))
    # add_test_imgs = []
    # test_images = glob('/mnt/mfs2/ailun.li/pingan_deepfacelab/deepfacelab_test/all_test_crop/*')
    # for img in test_images:
    #     add_test_imgs.append([img, 9])
    # dataset_test_other.imgs = add_test_imgs  # 增加deepfacelab
    logging.info(f'Train data :{len(dataset_train)}')
    logging.info(f'Val data :{len(dataset_val)}')
    logging.info(f'Test data :{len(dataset_test)}')
    # logging.info(f'Test other data :{len(dataset_test_other)}')
    if args.DDP:
    ...
    # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=3,T_mult=2,eta_min=args.min_lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if ema and  epoch == args.warmup_epochs + 1:
            from copy import deepcopy
            ema.ema = deepcopy(model)
            ema.ema.eval()
        train_stats = train_one_epoch_mullabel(
        ...
        scheduler=None)
        """
        every batch update lr is smooth
        """
        ...
        if True:
            val_stats = evaluate_mullabel(data_loader_val, model, criterion, args.device, bi_class_weight=args.bi_class_weight)
            test_stats = evaluate_mullabel_test(data_loader_test, model, criterion, args.device, bi_class_weight=args.bi_class_weight)
            if ema and epoch > args.warmup_epochs:
                logging.info('  ************ EMA ************  ')
                val_ema_stats = evaluate_mullabel_test(data_loader_val, ema.ema, criterion, args.device,bi_class_weight=args.bi_class_weight)
                test_ema_stats = evaluate_mullabel_test(data_loader_test, ema.ema, criterion, args.device,bi_class_weight=args.bi_class_weight)
            ...
            #if max_TNR == max(val_stats["tnr_0.002"]['TNR'],val_ema_stats["tnr_0.002"]['TNR'] if ema and epoch > args.warmup_epochs else 0):
...
if __name__ == '__main__':
    ...
    args.output_dir = f'run/{data[1]}_{data[2]}_RepVGG_MultiLabel_Nobias'

    if not args.eval:
        args.output_dir += f'_AllW_Dg_ClsW151_weight{args.weight}_size{args.input_size}'
        if args.resume:
        ...
        shutil.copy('./main_all_mullablel.py', args.output_dir + '/main_all_mullablel.py')
