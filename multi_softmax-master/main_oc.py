...
from head.OC_Softmax import  OC_Softmax
...
class FaceModel(torch.nn.Module):
    ...
    def __init__(self, backbone_factory, head_factory,multi_head=False):
        ...
        # self.head =  torch.nn.Linear(head_factory.head_param['feat_dim'],24,bias=False)
        # torch.nn.init.xavier_uniform(self.head.weight)
        if self.multi_head:
            self.head_binary = OC_Softmax(head_factory.head_param['feat_dim'], margin_r=args.m_r,margin_f=args.m_f,scale=args.s)
...
def get_args_parser():
    ...
    parser.add_argument('--s', default=20,type=float, help='admsoftmax s')
    parser.add_argument('--m_r', default=0.9,type=float, help='admsoftmax m of real')
    parser.add_argument('--m_f', default=0.2, type=float, help='admsoftmax m of fake')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_oc(
        ...
        if True:
            test_stats = evaluate_oc(data_loader_val, model, criterion, args.device, args.s, args.bi_class_weight)
            logging.info(
...
if __name__ == '__main__':
    ...
    if not args.eval:
        ...
        args.output_dir += f'_OC_s{args.s}_mr{args.m_r}_mf{args.m_f}_PIL_Dg_sizes{args.input_size}'
        if args.resume:
        ...
        shutil.copy('./main_oc.py', args.output_dir + '/main_oc.py')
