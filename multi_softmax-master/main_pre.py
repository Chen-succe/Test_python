...
class FaceModel(torch.nn.Module):
    ...
    def __init__(self, backbone_factory, head_factory,multi_head=False):
        ...
        # self.head =  torch.nn.Linear(head_factory.head_param['feat_dim'],24)
        # torch.nn.init.xavier_uniform(self.head.weight)
        if self.multi_head:
            self.head_binary = ADM_Softmax(head_factory.head_param['feat_dim'], 2,margin_r=args.m_r,margin_f=args.m_f,scale=args.s)
            # self.head_binary = torch.nn.Parameter(torch.Tensor(head_factory.head_param['feat_dim'], 2))
            # self.head_binary.data.uniform_(-1, 1).renorm_(2, 1, 1e-5).mul_(1e5)
            # self.head_binary = torch.nn.Linear(head_factory.head_param['feat_dim'],2,bias=False)
            # torch.nn.init.xavier_uniform(self.head_binary.weight)
    def forward(self, data, label):
        feat = self.backbone.forward(data)
        pred = self.head.forward(feat, label)
        # pred = self.head(feat)
        if self.multi_head:
            # kernel_norm = F.normalize(self.head_binary, dim=0)
            # feats = F.normalize(feat)
            # cos_theta = torch.mm(feats, kernel_norm)
            # cos_theta = cos_theta.clamp(-1, 1)
            # output = cos_theta * 1.0
            label[label>0]=1
            output = self.head_binary(feat,label)
...
def get_args_parser():
    ...
    parser.add_argument('--m_r', default=0.4,type=float, help='admsoftmax m of real')
    parser.add_argument('--m_f', default=0.1, type=float, help='admsoftmax m of fake')
...
def main(args):
    ...
    if args.finetune:
        ...
        # checkpoint_model.pop('head_binary.weight')
        checkpoint_model['head_binary.weight'] = checkpoint_model['head_binary.weight'].T
...
if __name__ == '__main__':
    ...
    args.output_dir = f'results/{data[1]}_{data[2]}_{args.backbone_type}_{args.head_type}'
    ...
    if not args.eval:
        ...
        args.output_dir += f'_PreTrained_AdM_s{args.s}_mr{args.m_r}_mf{args.m_f}_PIL_Dg_ClsW91_W{args.weight}_size{args.input_size}'
        ...
        if not args.resume:
            logging.info(args)
            shutil.copy('./main_pre.py',args.output_dir+'/main_pre.py')
