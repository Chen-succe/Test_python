...
def evaluate_test(data_loader, model, criterion, device, scale=0,bi_class_weight=[1,1], weight=1.0):
    ...
    for threshold_r in threshold_rs:
        ...
        abs_path = '/mnt/mfs2/ailun.li/wubao_shiyan/'
        for path in ['dandong_test','pingan_test','zhaji_test']: #['w_test','zhaji_zhengchang','ZY_wuju_baidu','ZY_wuju_KS','pingan_pos'
            ...
            batch_size = 1 if size == 224 else 1
            ...
            with torch.no_grad():
                for images, labels in tqdm(val_loader, desc='Evaluate'):
                    ...
                    FN += (score > thresh).sum().item()
                    if len(images[score > thresh]) > 0:
                        bad_case.append(images[score > thresh])
                    total_nums += bs
            FNR = FN / total_nums * 100
            tnr_info[f'FNR_{path}'] = FNR
