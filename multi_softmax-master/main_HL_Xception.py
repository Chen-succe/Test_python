...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--train_list', default='/home/ailun.li/apronet/txt/all/all_train.txt', type=str,help='dataset path')
    parser.add_argument('--train_add_list', default='/home/ailun.li/Multi_softmax/data_txts/197_多分类标签/w_train.txt', type=str, help='dataset path')
    ...
    parser.add_argument('--val_list', default='/home/ailun.li/apronet/txt/all/all_val.txt', type=str,help='dataset path')
    # /mnt/mfs2/ailun.li/data_txt/all/huanlian/newlabel/huanlian_val_newlabel.txt'
    # data_txts/all_test_PIL_resize224.txt
    parser.add_argument('--test_list', default='/home/ailun.li/apronet/txt/all/all_test.txt', type=str, help='dataset path')
    ...
    parser.add_argument('--num_classes', default=46,type=int, help='num classes')
    parser.add_argument('--s', default=5,type=int, help='amsoftmax s')
    ...
    parser.add_argument('--bi_class_weight', default=[15,1], type=list, help="Binary classify weight")
    parser.add_argument('--RDrop', default=0.0, type=float, help="RDrop, Only dropout in Model")
    parser.add_argument('--eval_savefail', action='store_true', help='save FP image at inference time')
    # * Finetuning params
...
def main(args):
    ...
    if args.resume:
        ...
        """ SWA """
        # checkpoint = torch.load(args.resume + f'epoch_{59}.pth', map_location='cpu')
        # n = 1
        # for swa in [67]:
        #     checkpoint_ = torch.load(args.resume+f'epoch_{swa}.pth', map_location='cpu')
        #     n+=1
        #     for k,v in checkpoint_['model'].items():
        #         checkpoint['model'][k]+=v
        # for k,v in checkpoint['model'].items():
        #     checkpoint['model'][k] = v/n
        ...
        if args.ema:
            ema = ModelEma(model,device=args.device)
    if args.eval:
        test_stats = evaluate_test(data_loader_val, model, criterion, args.device, args.s)
        print(test_stats)
        logging.info(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if True:
            val_stats = evaluate(data_loader_val, model, criterion, args.device, args.s,args.bi_class_weight)
            test_stats = evaluate_test(data_loader_test, model, criterion, args.device, args.s,args.bi_class_weight)
            logging.info(f"Accuracy of the network on the {len(dataset_val)} test images, acc multi: {val_stats['acc_multi']:.1f} acc bi: {val_stats['acc_bi']:.1f} ")
            logging.info( f"Accuracy of the network on the {len(dataset_test)} test images, acc bi: {test_stats['acc_bi']:.1f} ")
            if ema and epoch> args.warmup_epochs:
                logging.info('  ************ EMA ************  ')
                val_ema_stats = evaluate_test(data_loader_val, ema.ema, criterion, args.device, args.s, args.bi_class_weight)
                test_ema_stats = evaluate_test(data_loader_test, ema.ema, criterion, args.device, args.s,args.bi_class_weight)
                logging.info(f"Accuracy of the network on the {len(dataset_val)} test images, acc bi: {val_ema_stats['acc_bi']:.1f}")
                logging.info(f"Accuracy of the network on the {len(dataset_test)} test images, acc bi: {test_ema_stats['acc_bi']:.1f} ")
...
if __name__ == '__main__':
    ...
    args.output_dir = f'results/{data[1]}_{data[2]}_Xception_MultiSoftmax' #_{args.head_type}'

    if not args.eval:
        args.output_dir += f'_All_Dg_ClsW151_weight{args.weight}_RDrop_{args.RDrop}_size{args.input_size}'
        if args.resume:
        ...
        shutil.copy('./main_HL_Xception.py', args.output_dir + '/main_HL_Xception.py')
    ...
    main(args)
# python main_HL_Xception.py --gpus 0,1,2,3,4,5,6,7 --multi_head  --input-size 320 --batch-size 200  --ema --RDrop 0.3
