...
class RepVGG(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes=1000, width_multiplier=None, override_groups_map=None, deploy=False, use_se=False, use_checkpoint=False):
        ...
        if self.use_checkpoint:
            state_dict = torch.load(self.use_checkpoint, map_location='cpu')
            del state_dict['linear.weight']
            del state_dict['linear.bias']
            self.load_state_dict(state_dict)
            print('Load pretrained weight successfully')
        #self.linear = nn.Linear(int(512 * width_multiplier[3]), num_classes)
        # self.W = torch.nn.Parameter(torch.randn(int(512 * width_multiplier[3]), num_classes), requires_grad=True)
    ...
    #     out_norm = torch.mm(out_norm,w_norm)
...
def create_RepVGG_A0(deploy=False, use_checkpoint=False, num_classes=1000):
...
def create_RepVGG_A1(deploy=False, use_checkpoint=False, num_classes=1000):
    return RepVGG(num_blocks=[2, 4, 14, 1], num_classes=num_classes,
...
def create_RepVGG_B0(deploy=False, use_checkpoint=False, num_classes=1000):
...
if __name__ == '__main__':
    model = create_RepVGG_B0(num_classes=10)
