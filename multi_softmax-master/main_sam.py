...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--train_list', default='./huokai_train_PIL_resize224.txt', type=str,help='dataset path')
    ...
    parser.add_argument('--val_list', default='./huokai_test_PIL_resize224.txt', type=str,help='dataset path')
    ...
    parser.add_argument('--bi_class_weight', default=[9,1], type=list, help="Binary classify weight")
    parser.add_argument('--RDrop', default=0.0, type=float, help="RDrop, Only dropout in Model")

    #SAM
    #SAM
    parser.add_argument('--rho', default=2.0, type=float,help='SAM rho')
    ...
    parser.add_argument('--opt', default='sgd', type=str, metavar='OPTIMIZER',help='Optimizer (default: "adamw"')
    ...
    parser.add_argument('--lr', type=float, default=1e-2, metavar='LR', help='learning rate (default: 2e-3)')
...
def main(args):
    ...
    dataset_train = CustomDataset(args.train_list, type='train', transforms=transform_train)
    dataset_val = CustomDataset(args.val_list, type='test', transforms=transform_test)
    if args.DDP:
        ...
        # data_loader_train = webdataset.WebLoader(
        #     dataset_train, batch_size=None, shuffle=False, num_workers=args.num_workers,#pin_memory=args.pin_mem,
        # )
        data_loader_train = torch.utils.data.DataLoader(
            dataset_train,
            batch_size=args.batch_size,
    ...
    # mean,std = get_mean_std_value(data_loader_train)
    # print(mean)
    # print(std)
    ...
    from util.trick import SAM
    sam_optimizer = SAM(optimizer,rho=args.rho,adaptive=args.adaptive)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_sam(
            ...
            sam_optimizer, args.device, epoch, loss_scaler,
...
if __name__ == '__main__':
    ...
    args.output_dir = f'results/{args.backbone_type}_{args.head_type}'
    with open(args.head_conf_file) as f:
        head_conf = yaml.load(f, Loader=yaml.FullLoader)
        head_param = head_conf[args.head_type]
    head_param.pop('feat_dim')
    head_param.pop('num_class')
    for k,v in head_param.items():
        args.output_dir += f"_{k}{v}"
    data = time.asctime().split()
    if not args.eval:
        # args.output_dir += '_SingleHead_PIL_Dg_ClsW91_June_5'
        args.output_dir += f'_MultiHead_PIL_Dg_ClsW91_RDrop{args.RDrop}_SAM{args.rho}_{data[1]}_{data[2]}'
        ...
        if not args.resume:
            logging.info(args)
            shutil.copy('./main_sam.py',args.output_dir+'/main_sam.py')
