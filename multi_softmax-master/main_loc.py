...
class FaceModel(torch.nn.Module):
    ...
    def forward(self, data, mask):
        ...
        if self.training:
            ...
            for l,f in enumerate(range(feat[4].shape[1])):
                print(f"stage 4 ,channel {l},feature map max {feat[4][0, l].max()}，mean {feat[4][0, l].mean()}")
                if l==9:
                    break
            for l,f in enumerate(feat[1:-1]):
                mask = self.max_pool(mask) # 4,4,112,112
                fm1 = (mask.flatten(2) @ f.flatten(2).transpose(-2,-1))/(mask.sum((-1,-2)).unsqueeze(-1))
                for m in range(fm1.shape[1]):
                    print(f"stage {l},mask{m},feature map max {fm1[0,m,:].max()}，mean {fm1[0,m,:].mean()}")
                # fm1 = self.IN[l](fm1)
                # for m in range(fm1.shape[1]):
                #     print(f"stage {l} after IN ,mask{m},feature map max {fm1[0,m,:].max()}，mean {fm1[0,m,:].mean()}")
                feats.append(fm1) # /(mask.sum([2,3]).unsqueeze(-1)+1e-8)
            feats = torch.cat(feats,dim=-1) # B,M,C+ ->
            # feats = self.attn(feats)
            # feats = self.in1(feats)
...
def get_args_parser():
    ...
    parser.add_argument('--train_add_list', default=['data_txts/8g4/huokai_train_PIL_resize224_crazytalknew.txt'], type=list,help='add dataset path')
    ...
    parser.add_argument('--val_add_list', default=['data_txts/8g4/huokai_test_PIL_resize224_crazytalknew.txt'],type=list, help='add dataset path')
...
if __name__ == '__main__':
    ...
    args.output_dir = f'run/{data[1]}_{data[2]}_{args.backbone_type}_LOC_SplitINnoAttenAvg_MultiSoftmax'
    if not args.eval:
        args.output_dir += f'_Test_NofilterLandmark_Dg_ClsW{args.bi_class_weight[0]}{args.bi_class_weight[1]}' \
                           f'_mw{args.mw}_bw{args.bw}_lw{args.lw}_size{args.input_size}_epoch{args.epochs}'
        if args.resume:
        ...
        shutil.copy('./main_loc.py', args.output_dir + '/main_loc.py')
...
"""
python main_loc.py --gpus 5,6 --batch-size 512 --ema --mw 0 --lw 0.1 
"""
